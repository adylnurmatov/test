...
public class ElementTreePanel extends JPanel implements CaretListener, DocumentListener, PropertyChangeListener, TreeSelectionListener {
    ...
    public ElementTreePanel(JTextComponent editor) {
        ...
        tree = new JTree(treeModel) {
            public String convertValueToText(Object value, boolean selected, boolean expanded, boolean leaf, int row, boolean hasFocus) {
                if (!(value instanceof Element)) return value.toString();
        ...
        tree.setCellRenderer(new DefaultTreeCellRenderer() {
            public Dimension getPreferredSize() {
                ...
                if (retValue != null) retValue.width += 15;
                return retValue;
            }
        });
    ...
    public void propertyChange(PropertyChangeEvent e) {
        if (e.getSource() == getEditor() &&
         e.getPropertyName().equals("document")) {
            JTextComponent editor = new = getEditor();
    ...
    public void insertUpdate(DocumentEvent e) { updateTree(e); }
    public void removeUpdate(DocumentEvent e) { updateTree(e); }
    public void changedUpdate(DocumentEvent e) { updateTree(e); }
        ...
        if (!updatingSelection) {
            ...
            for (int counter = 0; counter < rootCount; counter++) {
                ...
                while (start <= end) {
                    ...
                    if(start >= charElement.getEndOffset) start++;
                    else start  = charElement.getEndOffset;
                }
            }
    ...
    public static class ElementTreeModel extends DefaultTreeModel {
        ...
        public ElementTreeModel(Document document) {
            ...
            rootElements = document.getRootElement();
        }


        public Object getChild(Object parent, int index) {
            if (parent == root) return rootElements[index];
            return super.getChild(parent, index);
        }

        public int getChildCount(Object parent) {
            if (parent == root) return rootElements.length;
        ...
        public int getIndexOfChild(Object parent, Object child) {
            if (parent == root) {
                for (int counter = rootElements.length - 1; counter >= 0; counter--) {
                    if (rootElements[counter] == child) return counter;
                }
                return -1;
            }
            ...
            if(listenerList != null && node != null) {
                ...
                if (parent != null) {
                    ...
                    if(anIndex != -1) {
                        ...
                        cIndexc[0] = anIndex;
                        nodesChanged(parent, cIndexc);
                    }
                }
            }
        }

        protected TreeNode[] getPathRoot(TreeNode aNode, int depth) {
            TreeNode[] retNodes;
            if(aNode == null) {
                if(depth = 0) {
                    return null;
